...
def test_rollup_sums_hours_routes_and_statuses():
    rows = [
        HourRow(_hour(28, 16), "/api/search", 200, 5),
...
def test_parse_rows_reads_row_oriented_payload():
    payload = {
        "schema": {"fields": []},
        "data": [
            {"hour": "2026-08-28T16:00:00Z", "path": "/api/search", "status": 200, "requests": 4},
    ...
    assert rows[1].path is None and rows[1].status is None
