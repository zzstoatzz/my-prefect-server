...
import pytest
from mps import pi_local as pi
from pydantic import ValidationError

from flows import pi_agent_local as pi_agent
...
def test_flow_preserves_approval_and_passes_configuration(monkeypatch, agent, approval):
    ...
    assert pi_agent.pi_agent_local.fn("objective", agent=agent, instructions="purpose") == "done"
...
def test_rejected_prompt_never_launches(monkeypatch):
    ...
    with pytest.raises(ValueError, match="rejected"):
        pi_agent.pi_agent_local.fn("objective")
...
def test_flow_passes_only_selected_provider_credential(monkeypatch, provider):
    ...
    run = Mock(return_value="done")
    monkeypatch.setattr(pi_agent, "run_pi", run)
    pi_agent.pi_agent_local.fn("explain code", agent=pi_agent.Agent(provider=provider))
