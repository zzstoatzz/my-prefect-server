...
class BriefingItem(BaseModel):
    ...
    note: str = Field(
        examples=["stale 2 weeks, might be blocked", "ready to merge, just needs a rebase"]
    )
...
class Briefing(BaseModel):
    ...
    title: str = Field(
        description="2-3 word vibe", examples=["monday triage", "quiet week", "bug cluster"]
    )
    headline: str = Field(
        description="1-2 sentence summary of what's going on and what to pay attention to",
        examples=[
