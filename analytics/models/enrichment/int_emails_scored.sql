WITH scored AS (
    SELECT
        ...
        -- newsletters/marketing (List-Unsubscribe et al) are context, not action items
        CASE WHEN e.is_bulk THEN 0.25 ELSE 1.0 END AS bulk_penalty
    FROM {{ ref('stg_emails') }} e
    WHERE e.received_at != ''
)
SELECT *,
    ROUND((0.6 * recency_score + 0.4 * unread_score) * bulk_penalty, 4) AS importance_score
FROM scored
